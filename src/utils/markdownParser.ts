...
export const markdownParser = new MarkdownParser(
  ...
  new MarkdownIt(),
  {
    ...
    image: { node: 'image', getAttrs: (tok: any) => ({ src: tok.attrGet('src'), title: tok.attrGet('title') || null, alt: (tok.children && tok.children[0] && tok.children[0].content) || null }) },
...
export function parseMarkdown(markdown: string, schema: Schema, plugins: Plugin[] = []) {
