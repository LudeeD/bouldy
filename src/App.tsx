...
function AppContent({ onResetVault }: { onResetVault: () => void }) {
  ...
  const [mountedPanels, setMountedPanels] = useState<Set<PanelType>>(new Set(['editor', 'todos']));
    ...
    return (
      ...
      >
        <div className="flex-1 overflow-auto p-4 relative group">
          ...
          <button
            onClick={(e) => { e.stopPropagation(); closePanel(side); }}
          ...
          {mountedPanels.has('editor') && (
            <div className={type === 'editor' ? 'w-full h-full' : 'absolute top-0 left-0 w-full h-full opacity-0 pointer-events-none'}>
              <Editor />
            </div>
          )}
          {mountedPanels.has('todos') && (
            <div className={type === 'todos' ? 'w-full h-full' : 'absolute top-0 left-0 w-full h-full opacity-0 pointer-events-none'}>
              <TodoSpace />
            </div>
          )}
          {mountedPanels.has('calendar') && (
            <div className={type === 'calendar' ? 'w-full h-full' : 'absolute top-0 left-0 w-full h-full opacity-0 pointer-events-none'}>
              <CalendarView />
            </div>
          )}
          {mountedPanels.has('settings') && (
            <div className={type === 'settings' ? 'w-full h-full' : 'absolute top-0 left-0 w-full h-full opacity-0 pointer-events-none'}>
              <SettingsPanel
  ...
  return (
    <div className="h-screen flex bg-bg">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div data-tauri-drag-region className="flex items-center justify-between px-4 py-2 cursor-move bg-bg-dark border-border">
